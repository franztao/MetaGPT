...
requirements = r'当前运行环境是linux正在运行的容器中，当前FlagPerf的git库地址在"/home/htao/debug/step3/FlagPerf",数据和模型地址在"/home/htao/debug/step2/FlagPerf/data/llama3_8B/megatron_llama3/data_dir",调用工具一定不要有删除卸载等高危操作。当前执行环境是在容器内。'
...
async def test_program_execute():
    ...
    if os.path.exists(p2):
        ...
        p2 = r'/home/htao/debug/MetaGPT/metagpt/roles/data/project_reasoner/step3_input_0306.txt'
    with open(p2, "r", encoding="utf-8") as f:
        ls = f.readlines()
    memory_short = content + "\n" + "---" * 1 + "\n文件地址" + p2 + "\n文件中内容:\n" + ''.join(ls) + "\n" + "---" * 1

    p2 = r'C:\Users\m01216.METAX-TECH\Desktop\code\FlagPerf/blob/main/docs/utils/definitions/IN_CONTAINER_LAUNCH.md'
    if os.path.exists(p2):
        pass
    else:
        p2 = r'/home/htao/debug/step3/FlagPerf/docs/utils/definitions/IN_CONTAINER_LAUNCH.md'
    with open(p2, "r", encoding="utf-8") as f:
        ls = f.readlines()
    memory_long = content + "\n" + "---" * 1 + "\n文件地址" + p2 + "\n文件中内容:\n" + ''.join(ls) + "\n" + "---" * 1
    l=max(1,len(memory_long) )
...
#   /home/htao/miniconda3/envs/metagpt39/bin/python   /home/htao/debug/MetaGPT/tests/metagpt/roles/test_program_execute.py
# /home/htao/miniconda3/envs/metagpt/bin/python   /home/htao/debug/MetaGPT/tests/metagpt/roles/test_program_execute.py
